...
type ExtractCommand<T> = T extends CommandHandler<infer Command, any, any>
  ? Command
  : never;
...
export class Aggregate<
  TState = unknown,
  TCommandHandler extends CommandHandler<
    Command,
    Event,
    TState
  > = CommandHandler<Command, Event, TState>,
  TEventHandler extends EventHandler<Event, TState> = EventHandler<
    Event,
    TState
  >
> {
  ...
  private async digest(events: AsyncIterable<Event> | Array<Event>) {
  ...
  private async dispatch(
    ...
    if (this.shoudTakeSnapshot()) {
      this.store
        ...
        .catch((err) => {
          this.logger.warn(
            `error occured while saving snapshot: error="${err.message}"`
          );
        });
    }
  }
  ...
  public async process(
    command: ExtractCommand<TCommandHandler>,
    ctx?: Buffer,
    opts?: {
      noReload?: true;
      maxRetries?: number;
          ...
          await this.dispatch(
            {
              ...
              events: (event instanceof Array ? event : [event]).map(
                (item) => ({
                  id: EventId.generate(),
                  type: item.type,
                  body: item.body,
